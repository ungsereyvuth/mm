...
class item_view{
	public function data($input){
		...
		$nearbyrow = $qry->qry_assoc("SELECT i.*,type.code typename,(datediff(i.end_date,i.start_date)+1) days,IFNULL(it.title_t, i.title ) title,IFNULL(it.description_t, i.description) description from v_items i 
									left join v_items_t it on it.main_id=i.id and it.language_id=$lang->id
									left join layout_text_item type on type.id=i.type_id
									left join layout_text_item_t type_t on type_t.item_id=type.id
									left join layout_text_item cate on CONCAT(',',i.cate_id,',') like CONCAT('%,',cate.id,',%')
									left join layout_text_item_t cate_t on cate_t.item_id=cate.id and cate_t.language_id=$lang->id
									where type.id=".$item['type_id']." $more_sql and i.id<>$item_id and i.active=1 and i.approved=1 and i.active_by_user=1 
									GROUP BY i.id");		
		...
		$places_row = $qry->qry_assoc("SELECT i.*,type.code,(datediff(i.end_date,i.start_date)+1) days,IFNULL(it.title_t, i.title ) title,IFNULL(it.description_t, i.description) description from v_items i 
									...
									left join layout_text_item type on type.id=i.type_id
									left join layout_text_item_t type_t on type_t.item_id=type.id
									where i.type_id in (".implode(',',$listing).") and i.active=1 and i.approved=1 and i.active_by_user=1");
