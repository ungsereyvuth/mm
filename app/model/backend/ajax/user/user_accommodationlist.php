...
class user_accommodationlist{
	public function data($data){
		...
		$sql_statement = "SELECT i.*,IFNULL(IFNULL(u.company_name,fullname),'N/A') company_name,role.title rolename,IFNULL(it.title_t,i.title) itemname,IF('$lang->selected'='kh',c.name_kh,c.name_en) provincecity_name,GROUP_CONCAT(IFNULL(cates_t.title,cates.title) separator ' - ') cate_title from v_items i
							...
							where type.code='accommodations' $sql_condition 
							GROUP BY i.id
		...
		foreach($rowData as $key=>$value){	
			...
			foreach($item_row as $lvalue){
				$translation_btn .= '<li><a class="load_form_btn" href="#" data-cmd="'.$load_data_cmd['load_item_t'].'" data-target_formid="newaccommodationtranslate" data-recordid="'.encodeString($value['id'].'_'.$lvalue['id'],$encryptKey).'"><img src="/assets/frontend/img/flags/'.$lvalue['code'].'.png" height="15"> '.$lvalue['title'].'</a></li>';
			}
