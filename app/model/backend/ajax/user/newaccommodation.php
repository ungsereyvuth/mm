...
class newaccommodation{
	public function data($data){
		...
		$reg_fields = array('text'=>array(	'recordid'=>addslashes($_POST['recordid']),
											...
											'price_from'=>addslashes(trim($_POST['price_from'])),
											'price_to'=>addslashes(trim($_POST['price_to'])),
		...
		$opt_fields = array('recordid','publish_now','top_attraction','filename','map_lat','map_lng','map_zoom','transportation_info','contact_info','price_from','price_to');
		...
		if(!count($err_fields)){
			...
			$sql = "title='".$reg_fields['text']['title']."',
					...
					price_from='".$reg_fields['text']['price_from']."',
					price_to='".$reg_fields['text']['price_to']."',
