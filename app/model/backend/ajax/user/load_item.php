...
class load_item{
	public function data($data){
		...
		if(is_numeric($recordid) and $recordid>0){
			$fromdata = $qry->qry_assoc("select * from v_items where id=$recordid limit 1");
			if(count($fromdata)){
				$fromdata = $fromdata[0];
				...
				$file_input_id = 'attachment';$filenames=array();
				foreach($files as $key=>$f){
					$value=$f->filename;
					if(trim($value)<>''){$filenames[]=$value;
				...
				$fromdata['cate_id[]'] = json_encode(explode(',',$fromdata['cate_id']));	
				$fromdata['file_preview'] = $file_preview;	
				$fromdata['publish_now'] = $fromdata['active'];	
				$fromdata['filename'] = implode('|',$filenames);	
				$map = explode(',',$fromdata['map']);	
				$fromdata['map_lat'] = $map[0];
				$fromdata['map_lng'] = $map[1];
				$fromdata['map_zoom'] = $map[2];	
