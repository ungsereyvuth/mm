<?php
class newElementTranslation{
	public function data($data){
		global $encryptKey,$language,$usersession,$layout,$layout_label,$lang;
		$qry = new connectDb; $_POST=$data;
		$refresh_listname='labelList'; $table = 'form_layout_element_t'; $parent_key = 'element_id';
		...
		$reg_fields = array('text'=>array(	'recordid'=>addslashes($_POST['recordid']),
											...
											'placeholder_t'=>addslashes($_POST['placeholder_t']),										
		...
		if(!count($err_fields)){
			...
			if(is_numeric($recordid) and $recordid and is_numeric($language_id) and $language_id){
				$prev_data = $qry->qry_assoc("select id from $table where $parent_key=$recordid and language_id=".$reg_fields['text']['language_id']." ");
		...
		if(!count($err_fields)){			
			...
			$sql = "title_t='".$reg_fields['text']['title_t']."',	
					placeholder_t='".$reg_fields['text']['placeholder_t']."',					
					active=".$reg_fields['text']['active'].",";
			if($isNew){
				$qry->insert("insert into $table set $parent_key=$recordid,language_id=$language_id,$sql created_by=".$usersession->info()->id.",created_date='$datetime'");
			}else{
				$qry->update("update $table set $sql last_updated_by=".$usersession->info()->id.",last_updated_date='$datetime' 
								where $parent_key=$recordid and language_id=$language_id limit 1");				
			}			
			//add to user log			
