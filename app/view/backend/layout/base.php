...
if(count($pageArr)){
	foreach($pageArr as $key=>$value){
		if($key==(count($pageArr)-1)){
			if(is_array($value)){
				$breadcrumb_txt .= '<li class="active tooltips" title="'.strip_tags($value['title']).'">'.((mb_strlen(strip_tags($value['title']),"utf-8")>20)?(mb_substr(strip_tags($value['title']),0,20,"utf-8").'...'):strip_tags($value['title'])).'</li>';
			}else{
				$bc_txt = 'Untitled';
				foreach($layout_cate as $cate_value){
					$catetitle = $cate_value['title'];
					$bc_txt = isset($pageData->label->$catetitle->$value->title)?strip_tags($pageData->label->$catetitle->$value->title):'Untitled';
			...
			if(is_array($value)){
				$breadcrumb_txt .= '<li><a href="/'.$selected_language.$value['url'].'">'.strip_tags($value['title']).'</a></li>';
			}else{
